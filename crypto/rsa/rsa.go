...
// cipher == "" 与 MarshalEncryptedPEM 等价（AES-256-CBC 兜底）；AEAD cipher
// （如 AES-GCM）不被 PKCS#8 PBES2 接受，调用将失败。失败时返回包装 OpError 的错误。
//
...
// (AES-256-CBC). AEAD ciphers (e.g. AES-GCM) are not accepted by PKCS#8
// PBES2 and cause a wrapped OpError on failure.
