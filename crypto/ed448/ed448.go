// Package ed448 基于铜锁原生实现封装 Ed448 签名算法（RFC 8032）。
// 提供密钥生成、PEM（PKCS#8 / SPKI）序列化、纯签名 / 验签，以及
// 57 字节原始私钥种子（seed）和公钥字节互操作。
//
// 摘要方面 Ed448 走"纯签名"：调用方传入的 msg 即被签内容，不在内部再做
// 预哈希——这与 RFC 8032 §5.2 的 Ed448 算法定义一致。
//
// Package ed448 wraps the Tongsuo-native Ed448 signature algorithm
// (RFC 8032). It exposes key generation, PEM (de)serialization for
// PKCS#8 private keys and SubjectPublicKeyInfo public keys, pure
// sign / verify (no internal pre-hashing), and a 57-byte raw private-
// seed / public-key byte interop.
//
// Ed448 signs messages directly without any digest pre-processing; the
// algorithm defined in RFC 8032 §5.2 absorbs the message bytes verbatim, and
...
//
// seedSize is the raw private-seed byte length of Ed448 (RFC 8032 §5.2.2).
const seedSize = 57

// ErrInvalidSeedLength 表示传入的原始私钥种子长度不是 Ed448 规定的 57 字节。
//
// ErrInvalidSeedLength reports that a supplied raw private seed does not
// have the 57-byte length mandated by Ed448.
var ErrInvalidSeedLength = fmt.Errorf("ed448: invalid seed length, want %d bytes", seedSize)

// ErrInvalidPublicKeyLength 表示传入的原始公钥字节长度不是 Ed448 规定的 57 字节。
//
// ErrInvalidPublicKeyLength reports that a supplied raw public key does
// not have the 57-byte length mandated by Ed448.
var ErrInvalidPublicKeyLength = fmt.Errorf("ed448: invalid public key length, want %d bytes", seedSize)

// PrivateKey 表示 Ed448 私钥，底层持有 *core.PKey 句柄。
//
// PrivateKey is an Ed448 private key backed by an internal *core.PKey.
type PrivateKey struct {
	key *core.PKey
}

// PublicKey 表示 Ed448 公钥，底层持有 *core.PKey 句柄。
//
// PublicKey is an Ed448 public key backed by an internal *core.PKey.
type PublicKey struct {
	key *core.PKey
}

// GenerateKey 生成新的 Ed448 签名密钥对。
//
// 底层失败时返回包装 OpError 的错误。
//
// GenerateKey generates a fresh Ed448 signing key pair.
//
...
// PrivateKeyFromSeed 从 57 字节种子构造私钥（种子来自 RNG 或外部熵源）。
//
// seed 长度必须为 57 字节；其他长度返回 ErrInvalidSeedLength。
// 调用方在调用后须自行清零 seed。
//
// PrivateKeyFromSeed constructs an Ed448 private key from a 57-byte
// raw private seed.
//
// seed must be exactly 57 bytes; other lengths return ErrInvalidSeedLength.
...
// PublicKeyFromBytes 从 57 字节公钥字节构造公钥。
//
// raw 长度必须为 57 字节；其他长度返回 ErrInvalidPublicKeyLength。
//
// PublicKeyFromBytes constructs an Ed448 public key from 57 raw bytes.
//
// raw must be exactly 57 bytes; other lengths return ErrInvalidPublicKeyLength.
...
// 对一侧执行的密码学操作在另一侧也可观察到（Ed448 私钥本身即携带派生公钥）。
//
...
// (Ed448 private keys carry the derived public key internally).
func (k *PrivateKey) Public() (*PublicKey, error) {
	if k == nil || k.key == nil {
		return nil, fmt.Errorf("ed448: nil private key")
	}
...
//
// Seed exports the 57-byte raw private seed.
//
...
//
// Bytes exports the 57-byte raw public key bytes.
//
...
func (k *PublicKey) Bytes() ([]byte, error) {
	if k == nil || k.key == nil {
		return nil, fmt.Errorf("ed448: nil public key")
	}
	return k.key.RawPublicKey()
}

// LoadPrivateKeyPEM 从 PEM（PKCS#8）加载 Ed448 私钥。
//
// 解析 PKCS#8（"-----BEGIN PRIVATE KEY-----"）PEM 块；底层走 EVP 通用路径，
// 自动识别 Ed448 算法。失败时返回包装 OpError 的错误。
//
// LoadPrivateKeyPEM parses an unencrypted PEM block carrying a PKCS#8
// ("-----BEGIN PRIVATE KEY-----") Ed448 private key. The Tongsuo EVP
// pipeline auto-detects the Ed448 algorithm. On failure it returns an
...
// LoadPublicKeyPEM 从 PEM（SubjectPublicKeyInfo）加载 Ed448 公钥。
//
// 解析 SPKI（"-----BEGIN PUBLIC KEY-----"）PEM 块；底层走 EVP 通用路径。
// 失败时返回包装 OpError 的错误。
//
// LoadPublicKeyPEM parses an unencrypted PEM block carrying a
// SubjectPublicKeyInfo ("-----BEGIN PUBLIC KEY-----") Ed448 public key.
// The Tongsuo EVP pipeline auto-detects the Ed448 algorithm. On failure
...
// LoadEncryptedPEM 从加密 PEM 加载 Ed448 私钥。
//
...
// and returns the underlying Ed448 private key. An empty passphrase or
...
// cipher == "" 与 MarshalEncryptedPEM 等价。失败时返回包装 OpError 的错误。
//
...
// cipher == "" matches MarshalEncryptedPEM.
...
// Sign 使用 Ed448 对 msg 签名，返回 114 字节签名。
//
// msg 即被签内容；Ed448 走 RFC 8032 §5.2 定义的"纯签名"语义，调用方传入的
// msg 不会被本函数再次预哈希（调用方**不应**对 msg 做预哈希）。priv 为 nil
// 或底层为 nil 时返回错误；底层签名失败返回包装 OpError 的错误。
//
// Sign produces an Ed448 signature over msg and returns 114 bytes.
//
// Ed448 uses the "pure" signature semantics defined in RFC 8032 §5.2;
// the supplied msg bytes are signed verbatim and this function does NOT
// pre-hash them (callers MUST NOT pre-hash msg either, or interop with
// other Ed448 implementations breaks). priv or priv.key nil returns an
...
// Verify 使用 Ed448 验签（sig 须为 114 字节 Ed448 签名）。
//
// pub 为 nil、底层为 nil 或签名长度/格式错误时返回错误（永不返回 bool）。
// 调用方应将任意非 nil 错误视为认证失败，无需进一步区分原因。
//
// Verify validates an Ed448 signature sig (must be 114 bytes) against msg.
//
...
func Verify(pub *PublicKey, msg, sig []byte) error {
	if pub == nil || pub.key == nil {
		return fmt.Errorf("ed448: nil public key")
	}
